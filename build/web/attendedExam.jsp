<%-- 
    Document   : attendedExam
    Created on : 27-Apr-2021, 4:18:59 pm
    Author     : Dhruv
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@page import="srcPackage.Exams"%>
...
<body style="background-color: #E7E9EB;">
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarText">
        ...
        <ul class="navbar-nav fs-5">
          ...
          <li class="nav-item px-2">
            <a class="nav-link active" aria-current="page" href="attendedExam.jsp">Attended Exam</a>
          </li>
        </ul>
    ...
    <h2><strong>Previously Attended Exams</strong></h2>
    <div class="row px-auto">
        <%
            ArrayList<Exams> list=pDAO.getAllAttendedExam(session.getAttribute("userId").toString());
            Exams exam;
        ...
        <div style="border: 3px solid #3FCE65; width: 400px;" class="card m-2">
            <div class="card-body text-center">
                <h2><%= exam.getEName()%></h2>
                <h5>Marks: <%= exam.getObtMarks()%> / <%= exam.getTMarks()%></h5>
                <h5>Date: <%= exam.getDate()%></h5>
                <h5>Time required: <%= exam.getExamTime()%> minutes</h5>
                <a href="examPage.jsp?eName=<%= exam.getEName()%>" class=" btn btn-primary my-3">Try Again</a>
            </div>
        </div>
        <%
            } 
